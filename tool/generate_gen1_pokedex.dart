...
Future<void> main() async {
  ...
  try {
    ...
    _write('lib/features/journal/data/red_pokedex_generated.dart','redGeneratedSpecies','Red',entries,rbLearn,rbMachines,redEnc,moveNames);
    _write('lib/features/journal/data/blue_pokedex_generated.dart','blueGeneratedSpecies','Blue',entries,rbLearn,rbMachines,blueEnc,moveNames);
    _write('lib/features/journal/data/yellow_pokedex_generated.dart','yellowGeneratedSpecies','Yellow',entries,yLearn,yMachines,yellowEnc,moveNames);
    ...
    stdout.writeln('Spanish move names: ${moveNames.length}/165.');
...
Map<int,String> _entries(File f,Map<String,int> ids){
  ...
  for(var i=0;i<labels.length;i++){
    ...
    final pieces=<String>[];
    for(final m in RegExp(r'(?:text|next|line|para|cont|page)\s+"([^"]*)"').allMatches(block)){ pieces.add(m.group(1)!); }
    if(pieces.isNotEmpty) out[id]=pieces.join(' ').replaceAll('@','').replaceAll('#MON','Pokémon').replaceAll('# BALL','Poké Ball').replaceAll(RegExp(r'\s+'),' ').trim();
  } return out;
}

Map<String,String> _moveNames(Directory root){
  final constants=File('${root.path}/constants/move_constants.asm'); final names=File('${root.path}/data/moves/names.asm');
  if(!constants.existsSync()||!names.existsSync()) return {};
  final symbols=<String>[]; var started=false;
  for(final line in constants.readAsLinesSync()){
...
Map<int,List<String>> _machines(Directory dir,Map<String,int> ids){
  ...
  for(final f in dir.listSync().whereType<File>().where((f)=>f.path.endsWith('.asm'))){
    final t=f.readAsStringSync(); final dm=RegExp(r'DEX_([A-Z0-9_]+)').firstMatch(t); if(dm==null) continue; final id=ids[_norm(dm.group(1)!)]; if(id==null) continue;
    final m=RegExp(r'tmhm\s+([\s\S]*?)(?:; end|\n\s*db\s+0)').firstMatch(t); if(m==null){out[id]=[];continue;}
    out[id]=RegExp(r'[A-Z][A-Z0-9_]+').allMatches(m.group(1)!.replaceAll(r'\',' ')).map((x)=>x.group(0)!).toList();
  } return out;
}
...
Map<int,List<_Encounter>> _encounters(Directory dir,Map<String,int> ids,String version){
  ...
  for(final f in dir.listSync().whereType<File>()){
    ...
    for(final raw in f.readAsLinesSync()){
      final l=raw.trim(); final cond=RegExp(r'IF DEF\(_([A-Z]+)\)').firstMatch(l);
      if(cond!=null){stack.add(enabled); enabled=enabled&&cond.group(1)==version; continue;}
      if(l.startsWith('ELSE')){if(stack.isNotEmpty) enabled=stack.last&&!enabled;continue;}
      if(l.startsWith('ENDC')){if(stack.isNotEmpty) enabled=stack.removeLast();continue;}
      if(!enabled) continue;
      final m=RegExp(r'db\s+\d+\s*,\s*([A-Z][A-Z0-9_]*)').firstMatch(l); if(m==null) continue; final id=ids[_norm(m.group(1)!)]; if(id==null) continue;
      final list=out.putIfAbsent(id,()=>[]); if(!list.any((e)=>e.location==location)) list.add(_Encounter(location,'Hierba/agua'));
    }
  } return out;
}

void _write(String path,String variable,String version,Map<int,String> entries,Map<int,List<_Move>> learn,Map<int,List<String>> machines,Map<int,List<_Encounter>> enc,Map<String,String> moveNames){
